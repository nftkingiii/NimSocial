...
import {
  ...
  Compass, Feather, Grid2X2, Home, MapPin, Menu, Plus, Search, Settings, ShieldCheck, Sparkles,
...
import type { AppSection, FeedPost, PostKind, ProfessionalProfile, ProfileInput, WalletIdentity } from "./types";
...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <nav className="bottom-nav" aria-label="Mobile navigation">
      ...
      <MobileNavButton label="Wallet" active={false} onClick={wallet ? () => handleNavigation("profile") : connectWallet}><WalletCards /></MobileNavButton>
      <MobileNavButton label="Notifications" active={false} badge="2" onClick={() => setNotice({ tone: "info", message: "Notifications will surface follows, applications, milestones, and settlements here." })}><Bell /></MobileNavButton>
...
function FeedScreen({ posts, source, onCompose, onOpenProfile }: { posts: FeedPost[]; source: string; onCompose: () => void; onOpenProfile: (wallet: string) => void }) { const [tab,setTab]=useState<"home"|"following"|PostKind>("home");const visible=tab==="home"||tab==="following"?posts:posts.filter((post)=>post.kind===tab);return <><header className="screen-header"><div><h1>Work worth following.</h1><p>Find real requests, useful people, and visible progress.</p></div><button className="icon-button screen-header__bell" type="button" aria-label="Notifications"><Bell /></button></header><button className="composer-trigger" type="button" onClick={onCompose}><Avatar name="You" /><span>Share a request, service, or progress update…</span><span className="composer-trigger__action"><Plus size={18} />Post</span></button><div className="feed-controls"><div role="tablist" aria-label="Feed view">{[["home","Home"],["following","Following"],["request","Requests"],["service","Services"],["proof","Proof"]].map(([value,label])=><button key={value} className={tab===value?"is-active":""} type="button" role="tab" aria-selected={tab===value} onClick={()=>setTab(value as typeof tab)}>{label}</button>)}</div><button type="button"><Sparkles size={16} />Relevant first</button></div>{source !== "live" && <div className={`preview-banner ${source === "error" ? "preview-banner--warning" : ""}`}><Sparkles size={17} /><span><strong>Product preview</strong> — illustrative posts are shown until the live feed has activity.</span></div>}<section className="feed-stack" aria-label="Posts">{visible.map((post) => <PostCard key={post.id} post={post} onOpenProfile={onOpenProfile} />)}</section></>; }
