...
export function PostCard({ post, onOpenProfile, onViewJob, onOpenThread, onAction, detail = false }: { post: FeedPost; onOpenProfile?: (walletAddress:string)=>void; onViewJob?: (jobId:string)=>void; onOpenThread?: (post:FeedPost)=>void; onAction?: (action:Action, post:FeedPost, detail?:string)=>void; detail?:boolean }) {
  ...
  return (
    <article className={`post-card ${onOpenThread&&!detail?"post-card--openable":""} ${detail?"post-card--detail":""}`} onClick={openFromCard}>
      ...
      {onOpenThread&&!detail?<button className="post-card__body post-card__body-button" type="button" onClick={()=>onOpenThread(post)} aria-label="Open post and view replies">{post.body}</button>:<p className="post-card__body">{post.body}</p>}
      ...
      <footer className="post-card__footer">
        <div className="engagement" aria-label="Post actions">
          <button className={commenting?"is-active":""} type="button" aria-label={detail?"Write a reply":onOpenThread?"View replies":"Comment"} aria-expanded={detail||!onOpenThread?commenting:undefined} onClick={()=>detail||!onOpenThread?setCommenting((open)=>!open):onOpenThread(post)}><MessageCircle size={18} /><span>{commentCount}</span></button>
